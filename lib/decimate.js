...
function clusterOnce(positions, indices, gridSize, edgeFloor, colors) {
  ...
  for (let v = 0; v < positions.length / 3; v++) {
    ...
    if (colors) {
      entry.r = (entry.r ?? 0) + colors[v * 3];
      entry.g = (entry.g ?? 0) + colors[v * 3 + 1];
      entry.b = (entry.b ?? 0) + colors[v * 3 + 2];
    }
    entry.n += 1;
  ...
  const newColors = colors ? new Array(cells.size * 3) : null;
  for (const entry of cells.values()) {
    ...
    if (newColors) {
      newColors[entry.id * 3] = entry.r / entry.n;
      newColors[entry.id * 3 + 1] = entry.g / entry.n;
      newColors[entry.id * 3 + 2] = entry.b / entry.n;
    }
  }
  ...
  return { positions: newPositions, colors: newColors, indices: newIndices };
}

// Binary-search the grid resolution until the triangle count lands near the target.
export function decimateToTarget(positions, indices, targetTriangles, { colors = null } = {}) {
  const sourceTriangles = indices.length / 3;
  if (sourceTriangles <= targetTriangles) {
    // Under the target the master passes through untouched — no clustering, no artifacts.
    return { positions: positions.slice(), colors: colors ? colors.slice() : null, indices: indices.slice(), gridSize: null, exact: true };
  }
  ...
  for (const g of candidates) {
    const result = clusterOnce(positions, indices, g, edgeFloor, colors);
  ...
  if (!best) return { positions: positions.slice(), colors: colors ? colors.slice() : null, indices: indices.slice(), gridSize: null, exact: true };
