...
// --- Bazaar discovery ---------------------------------------------------------
//
// Each tier declares how to call it and what comes back, so an agent that has never
// seen Facet can work out from the catalogue alone whether this resource does what it
// needs — without paying anything to find out.
...
const MESH_OUTPUT = {
  ...
  triangleCount: 25272,
  vertexCount: 12774,
  positions: [0.5982, 0.0653, 0.0419],
  normals: [-0.8758, -0.1058, 0.4709],
  indices: [6, 30, 7],
  paid: true,
};

const OUTPUT_EXAMPLES = {
  draft: { ...MESH_OUTPUT, tier: 1, tierKey: "draft", triangleCount: 3760 },
  production: MESH_OUTPUT,
  pbr: {
    tier: 3,
    tierKey: "pbr",
    material: { name: "Diamond-cut alloy, satin graphite", rim: { color: "#c9ced6", metalness: 1, roughness: 0.22 } },
    paid: true,
  },
  license: {
    tier: 4,
    ...
    licence: {
      ...
      proof: "The settled Algorand transaction for this request is the licence receipt.",
    },
    paid: true,
  },
};

export function discoveryExtensionFor(key) {
  return declareDiscoveryExtension({
    method: "POST",
    // No input needed: the tier IS the route, so the URL fully identifies the purchase.
    input: {},
    inputSchema: { properties: {}, required: [] },
    bodyType: "json",
    output: { example: OUTPUT_EXAMPLES[key] },
  });
}

// Tags an agent can filter on. Kept generic on purpose — an agent looking for "3d-asset"
// should find Facet without knowing the project exists.
export const TAGS_FOR = {
  draft: ["3d-asset", "mesh", "low-poly", "thumbnail"],
  production: ["3d-asset", "mesh", "high-poly", "configurator", "ar"],
  pbr: ["3d-asset", "material", "pbr", "photoreal"],
  license: ["3d-asset", "licence", "commercial-use"],
};
