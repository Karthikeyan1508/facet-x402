...
import { TIERS } from "./geometry.js";
import { getMesh, getMaterial, triangleCountFor, IS_INGESTED, SOURCE_INFO } from "./asset.js";
import { bazaarResourceServerExtension } from "@x402/extensions/bazaar";
import { ASSET_ID, ASSET_NAME, ALGORAND_TESTNET, PAYEES, CATALOGUE, TIER_BY_KEY, discoveryExtensionFor, TAGS_FOR } from "./catalogue.js";
...
// Bazaar registration is opt-in. Off by default so the deployed behaviour is exactly
// what it was before discovery existed; set BAZAAR_ENABLED=1 to advertise these routes
// in the facilitator's public catalogue.
export const BAZAAR_ENABLED = process.env.BAZAAR_ENABLED === "1";
...
for (const item of CATALOGUE) {
  routes[`POST ${item.route}`] = {
    ...
    ...(BAZAAR_ENABLED
      ? { tags: TAGS_FOR[item.key], extensions: discoveryExtensionFor(item.key) }
      : {}),
  };
}
...
if (BAZAAR_ENABLED) {
  ...
  log("info", "Bazaar discovery enabled — these routes will be catalogued publicly");
}
...
async function payAndFetch(url) {
  ...
  try {
    ...
    // Bare "fetch failed" from undici says nothing useful. Name the target and the two
    // things that are actually ever wrong here.
    const hint = process.env.ASSET_SERVER_URL
      ? `ASSET_SERVER_URL is set to "${process.env.ASSET_SERVER_URL}" — unset it unless the seller really lives elsewhere.`
      : "Check that the Algorand node and the facilitator are reachable from this machine.";
    throw new Error(`Could not reach ${url} — ${err.message}. ${hint}`);
  }
...
export function createApp() {
  ...
  // Behind Vercel (or any reverse proxy) Express sees the internal hop, so req.protocol
  // reports "http" and the x402 middleware advertises an http:// resource URL — a URL that
  // does not actually work, and one a facilitator has good reason to skip when building a
  // public catalogue. Trusting the proxy makes x-forwarded-proto authoritative, so the
  // advertised URL matches the URL people can really reach.
  ...
  app.get("/api/catalog", (_req, res) => {
    res.json({
      ...
      freePreview: { tier: 0, route: `/api/asset/${ASSET_ID}/preview`, triangles: triangleCountFor(0), price: "$0.00" },
      assetSource: IS_INGESTED ? { ingested: true, ...SOURCE_INFO } : { ingested: false, generator: "procedural" },
  ...
  const servePreview = (_req, res) => {
    const mesh = getMesh(PREVIEW_TIER);
    res.json({
      ...mesh,
      watermark: PREVIEW_TIER === 0,
      paid: false,
      ...(PREVIEW_MATERIAL ? { devMaterial: getMaterial() } : {}),
    });
  };
  ...
  app.post(`/api/asset/${ASSET_ID}/draft`, paid(() => ({ ...getMesh(1), watermark: false, paid: true })));
  app.post(`/api/asset/${ASSET_ID}/production`, paid(() => ({ ...getMesh(2), watermark: false, paid: true })));
  ...
  app.post(`/api/asset/${ASSET_ID}/pbr`, paid(() => ({
    tier: 3, tierKey: "pbr", label: TIERS[3].label, material: getMaterial(), paid: true,
  })));
  ...
  app.post("/api/unlock/:tierKey", async (req, res) => {
    ...
    try {
      ...
      log("info", "unlocking tier", { tierKey: item.key, target });
